# -*- coding: utf-8 -*-
"""微信聊天记录批量导出工具 v2.1.0 —— 自绘界面版。

功能与 v2.0.0 完全一致（获取密钥 / 连接数据库 / 勾选会话 / 批量导出 /
进度取消 / 预览 / 编辑给 AI 的指令），只替换了界面层：
  · 背景：渐变 + 高斯模糊光晕（也支持放自己的图，见 ui_theme.find_bg_image）
  · 面板：玻璃拟态（背景与面板色混合后切圆角）
  · 控件：全部 Canvas 自绘，悬停/按下带缓动过渡
  · 深浅主题可切换，选择会记住
