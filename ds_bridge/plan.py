...
为什么需要它
------------
网页版的**单条消息**接不下太多附件：实测挂 40 个就会被拒收（每个附件显示
"服务器繁忙"、整条消息报"请删除异常文件再发送"），30 个以内才稳。而一次导出
可能是「几个 .md + 上千张图」，所以发送前必须把文件排好序、切成 ≤30 的批次：

    先发文档（给AI的指令 → 各会话 .md → 导出清单.html）
    再发图片（按会话、按编号），并且**跨会话凑满一批**（用户明确要求：
    前一个群剩 10 张时，拿下一个群的 20 张凑满 30）
...
DEFAULT_LIMIT = 30                     # 单条消息的附件数安全上限（实测 40 就被官网拒收）
