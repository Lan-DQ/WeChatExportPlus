...
http.createServer((req, res) => {
    ...
    try {
        ...
        if (a === 'messages') {
            ...
            if (out[0]) fs2(out[0]);
            // ZSTD 解压
            // [WeChatExportPlus 改动] 原上限为 10000，超过即放弃解压并把裸 hex 交回 Python，
            // 而 Python 侧导出器无法解析 hex，结果这条消息被静默丢弃（实测真实库中有 2 条）。
            // 这里把上限放宽到 1,000,000 字符：正常消息走 fzstd 解压路径，
            // 仍然保留「解压失败就回落原文」的行为，不改变其它逻辑。
            try {
                    ...
                    for (const f of ['message_content', 'compress_content']) {
                        ...
                        if (buf.length >= 4 && buf.readUInt32LE(0) === 0xFD2FB528) {
                            try {
                                ...
                                if (tm) t = tm[1];
                                if (t && t.length < 1000000) { msg.message_content = t; break; }
                            } catch(e) {}
                        }
                    }
                }
